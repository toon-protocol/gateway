...
describe('withDialRewrites', () => {
  it('dials the rewritten target, and the same dialler otherwise', async () => {
    ...
    try {
      ...
      const socket = dialer.connect('never.resolves.invalid', 80);
      assert.ok(socket, 'a rewritten dial is made here, so the caller cannot dial the advertised name');
  ...
  it('still refuses an .anyone host, unless that host is the one rewritten', () => {
    ...
    const socket = dialer.connect('x.anyone', 80);
