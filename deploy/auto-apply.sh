...
# --ignore-buildable: the `gateway` service has no image to pull, and without
# this a pull fails the whole apply on the one service the apply exists for.
...
# the door behind it answers, and a rebuilt gateway is the change most likely
# to have gone wrong.
