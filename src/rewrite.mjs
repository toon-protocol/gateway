...
/**
 * A direct dial made here rather than by the caller, as a promise of a
 * connected socket — the shape `src/dial.mjs` fixes for every dial, because
 * reaching an `.anyone` host cannot be synchronous and one seam cannot have
 * two shapes.
 *
 * @param {string} host
 * @param {number} port
 ...
 * it was going to.
 *
 * @template {{ connect: import('./dial.mjs').Dial }} D
 * @param {D} dialer
 * @param {Map<string, Endpoint>} rewrites
 * @returns {D}
 */
export function withDialRewrites(dialer, rewrites) {
  ...
  return {
    ...
    connect(host, port) {
      ...
      return dialer.connect(to.host, to.port) ?? directDial(to.host, to.port);
    },
  };
}
